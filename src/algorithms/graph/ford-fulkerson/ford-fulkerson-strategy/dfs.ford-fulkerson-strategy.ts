...
export class Dfs_FordFulkersonStrategy
  implements FordFulkersonStrategy_Interface
{
   ...
   *
   * This version use dfs to find Augmenting Path
   *
   * @param graph
   * @param start
   * @param end
  ...
  execute(graph: DirectedGraph, start: number, end: number): number {
    ...
    let augmentingPath = findAugmentingPath(residual, start, end);
    ...
    while (bottleneck > 0) {
      ...
      augmentingPath = findAugmentingPath(residual, start, end);
